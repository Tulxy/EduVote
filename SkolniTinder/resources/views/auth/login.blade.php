...
<head>
    ...
    <title>Přihlášení - EduVote</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link
        href="https://fonts.googleapis.com/css2?family=Bricolage+Grotesque:opsz,wght@12..96,700;12..96,800&family=DM+Sans:wght@300;400;500&display=swap"
        rel="stylesheet">
    <style>
        body {
        ...
        .glow-lime {
            box-shadow: 0 8px 24px rgba(201, 240, 80, .2);
        }
...
{{-- Glow blobs --}}
<div
    class="fixed -top-32 -right-32 w-[500px] h-[500px] rounded-full bg-[#C9F050]/[0.05] blur-[140px] pointer-events-none z-0"></div>
<div
    class="fixed -bottom-32 -left-32 w-[400px] h-[400px] rounded-full bg-[#FF6B52]/[0.06] blur-[140px] pointer-events-none z-0"></div>

{{-- ─── NAV ─── --}}
<nav class="relative z-10 flex items-center justify-between px-6 lg:px-16 py-4 border-b border-white/[0.06]">
    <a href="{{ url('/') }}"
       class="flex items-center gap-2 font-display font-bold text-[1.15rem] text-[#F2F1EC] no-underline">
        <span class="w-2.5 h-2.5 rounded-full bg-[#C9F050]"></span>
        EduVote
    </a>
    <a href="{{ route('register') }}"
       class="text-sm text-[#5C5F7A] hover:text-[#F2F1EC] transition-colors no-underline">
...
<main class="relative z-10 flex flex-1 items-center justify-center px-4 py-16">
    <div class="w-full max-w-md">
        ...
        <div class="text-center mb-10 fade-up">
            <div
                class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-medium text-[#C9F050] bg-[#C9F050]/10 border border-[#C9F050]/25 mb-6">
        ...
        @if (session('status'))
            <div
                class="mb-5 flex items-center gap-2 px-4 py-3 rounded-xl bg-[#C9F050]/10 border border-[#C9F050]/25 text-sm text-[#C9F050]">
        ...
        @if ($message === 'These credentials do not match our records.' || str_contains($message, 'nepodaří'))
            <div
                class="mb-5 flex items-center gap-2 px-4 py-3 rounded-xl bg-[#FF6B52]/10 border border-[#FF6B52]/25 text-sm text-[#FF6B52]">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 flex-shrink-0" viewBox="0 0 24 24" fill="none"
                     stroke="currentColor" stroke-width="2.5">
        ...
        <div class="fade-up-2 bg-[#1C1D2A]/60 backdrop-blur-sm border border-[#2E3046] rounded-2xl p-8">
            ...
            <form method="POST" action="{{ route('login') }}" class="flex flex-col gap-5">
                ...
                <div class="flex flex-col gap-1.5">
                    <label class="text-sm font-medium text-[#E8E7F0]">E-mail nebo Jméno</label>
                    <input
                        type="text" {{-- Změněno na text, aby prohlížeč nehlásil chybu u jména --}}
                    name="login" {{-- Změněno na login --}}
                        value="{{ old('login') }}"
                        required
                        autofocus
                        placeholder="jan.novak nebo jan@skola.cz"
                        class="input-field @error('login') error @enderror"
                    >
                    @error('login')
                    <span class="flex items-center gap-1.5 text-xs text-[#FF6B52]">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3 flex-shrink-0" viewBox="0 0 24 24" fill="none"
             stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12"
                                                                                            y2="12"/><line x1="12"
                                                                                                           y1="16"
                                                                                                           x2="12.01"
                                                                                                           y2="16"/></svg>
        {{ $message }}
                ...
                <div class="flex flex-col gap-1.5">
                    <div class="flex items-center justify-between">
                        ...
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}"
                               class="text-xs text-[#5C5F7A] hover:text-[#C9F050] transition-colors no-underline">
                                Zapomněl jsi heslo?
                            </a>
                        @endif
                    </div>
                    <div class="relative">
                        ...
                        <button type="button" onclick="togglePassword()"
                                class="absolute right-3 top-1/2 -translate-y-1/2 text-[#5C5F7A] hover:text-[#E8E7F0] transition-colors">
                            <svg id="eye-icon" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24"
                    ...
                    <span class="flex items-center gap-1.5 text-xs text-[#FF6B52]">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3 flex-shrink-0"
                                     viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle
                ...
                <label class="flex items-center gap-3 cursor-pointer group">
                    <div class="relative">
                        ...
                        <div
                            class="w-10 h-5 bg-[#2E3046] border border-[#2E3046] rounded-full peer-checked:bg-[#C9F050]/20 peer-checked:border-[#C9F050]/40 transition-all"></div>
                        <div
                            class="absolute top-0.5 left-0.5 w-4 h-4 bg-[#5C5F7A] rounded-full peer-checked:translate-x-5 peer-checked:bg-[#C9F050] transition-all"></div>
                    </div>
